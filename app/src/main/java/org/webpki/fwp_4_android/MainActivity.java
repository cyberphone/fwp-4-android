...
public class MainActivity extends AppCompatActivity {
    ...
    public void homeScreen() {
        ...
        addCommandButton(s, "FIDO Assert", "fidoAssert()");
    ...
    @JavascriptInterface
    public void fidoAssert() {
        loadHtml("", "Assert FIDO",
                "<textarea id='jsonData' style='width:100%;height:60%;word-break:break-all'>" +
                        "</textarea>" +
                        executeButton("doAssertFIDO(document.getElementById(\"jsonData\").value)"));
    }

    @JavascriptInterface
    public void doAssertFIDO(String jsonData) {
        try {
            ArrayList<String> credentialId = new ArrayList<>();
            credentialId.add(jsonData.trim());
            FidoSupport.getSignRequest(credentialId);
            loadHtml("", "FIDO Assertion Result",
                    jsonData);
        } catch (Exception e) {
            loadHtml("", "FIDO Exception",
                    e.getMessage());
        }
    }
